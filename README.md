...
- **图片预览直接复用**：插图、截图、扫描页需要细看时，接入内置离线图片预览包，支持缩放、拖动、触屏双指缩放、适应窗口及返回原阅读位置。复制成品文件即可，日常接入不用安装依赖或阅读源码；配色沿用当前项目。见[三步接入说明](./codex-skill/adaptive-course-teacher/references/image-preview.md)。
